  ...
  useEffect(() => {
    const fetchTrendingFashion = async () => {
      try {
        ...
        const products = querySnapshot.docs
          .map((doc) => {
            ...
            return {
              ...
              categoryId: data.category === 'Clothing' ? 2 : null,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (!product || product.category !== 'Clothing' || product.rating < 4.5) {
              return false;
            }
        ...
        console.log('Fetched trending fashion products:', products);
        setTrendingProducts(products);
      } catch (err) {
        console.error('Error loading trending fashion products:', err);
        setError('Failed to load trending fashion products.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/products?category=2" className="text-blue-600 hover:underline text-sm">
          See All
        </Link>
      </div>
      {error ? (
